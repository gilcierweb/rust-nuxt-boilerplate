...
import { stringifyJson } from '~/utils/admin-ui'
...
const { itemId, item: rawItem, pending, error, refresh } = useAdminResourceItem<AuditLogItem>('audit-logs', { keyPrefix: 'admin-audit-logs-edit' })

const item = computed(() => {
  const log = rawItem.value
  if (!log) return null
  return {
    ...log,
    ...
    changes: stringifyJson(log.changes),
