...
export const ADMIN_RESOURCES: AdminResourceConfig[] = [
  ...
  },
  {
    ...
    canShow: true,
    fields: [
      lookupField('company_id', 'Empresa', 'companies'),
      lookupField('actor_user_id', 'Usuário executor', 'users'),
      { key: 'actor_role_snapshot', label: 'Perfil capturado' },
      { key: 'action', label: 'Ação', required: true, placeholder: 'payment.approved' },
      { key: 'resource_type', label: 'Tipo de recurso', required: true, placeholder: 'payment' },
      idField('resource_id', 'ID do recurso'),
      lookupField('target_customer_id', 'Cliente alvo', 'customers'),
    ...
    columns: [
      ...
      { key: 'target_customer_id', label: 'Cliente', lookupEntity: 'customers' },
