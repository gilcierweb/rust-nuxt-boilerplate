...
async fn build_runtime(config: &Arc<AppConfig>, boot_start: std::time::Instant) -> AppRuntime {
    ...
    if config.redis_pool_size < crate::config::app_config::REDIS_POOL_MIN_PRODUCTION
        && matches!(
            config.environment,
        ...
        )
    {
        tracing::error!(
            ...
            pool_size = config.redis_pool_size,
            recommended = crate::config::app_config::REDIS_POOL_MIN_PRODUCTION,
            ...
            crate::config::app_config::REDIS_POOL_MIN_PRODUCTION
        );
    } else if config.redis_pool_size < crate::config::app_config::REDIS_POOL_RECOMMENDED_PRODUCTION
        && matches!(
            config.environment,
            crate::config::app_config::Environment::Production
        )
    {
        tracing::warn!(
            ...
            pool_size = config.redis_pool_size,
            recommended = crate::config::app_config::REDIS_POOL_RECOMMENDED_PRODUCTION,
            ...
            crate::config::app_config::REDIS_POOL_RECOMMENDED_PRODUCTION,
    ...
    let ws_state = web::Data::new(crate::ws::WsRedisState::new(
        redis_pool_for_ws,
        crate::ws::WsLimits::default(),
    ));
